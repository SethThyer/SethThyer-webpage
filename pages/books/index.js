export default function Books() {
  return (
    <div>
      <center>
        <h6>My Media Recommendations</h6>
      </center>
      <h5>- Some of my favourite media</h5>
      <h5>Books</h5>
      <ul>
        <item>Industrial Society and Its Future -Theodore J. Kaczynski</item>
        <br />
        <item>Thus Spoke Zarathustra -Friedrich Nietzsche</item>
        <br />
        <item>Beyond Good And Evil -Friedrich Nietzsche</item>
        <br />
        <item>Life 3.0 -Max Tegmark</item>
        <br />
        <item>The Republic -Plato</item>
        <br />
        <item>Fear and Trembling -Soren Kierkegaard</item>
        <br />
        <item>The Myth of the Rational Voter -Bryan Caplan</item>
        <br />
        <item>1984 -George Orwell</item>
        <br />
        <item>Maps of Meaning -Jordan B. Peterson</item>
        <br />
        <item>Metamorphosis -Franz Kafka</item>
        <br />
        <item>The Old Man and the Sea -Ernest Hemingway</item>
        <br />
        <item>Crime and Punishment -Fyodor Dostoevsky</item>
        <br />
        <item>The Idiot -Fyodor Dostoevsky</item>
        <br />
        <item>Against Method -Paul Feyerabend</item>
        <br />
        <item>On War -Carl von Clausewitz</item>
        <br />
        <item>The Interpretation of Dreams -Sigmund Freud</item>
        <br />
        <item>Confessions of a Greenpeace Dropout -Patrick Albert Moore</item>
        <br />
        <item>The Gulag Archipelago -Aleksandr Solzhenitsyn</item>
      </ul>
      <h5>Programming Books</h5>
      <ul>
        <item>
          Introduction to Algorithms -Thomas Cormen, Charles Leiserson
        </item>
        <br />
        <item>Node.js in Action -Alex Young, Bradley Meck, Mike Cantelon</item>
        <br />
        <item>JavaScript: The Good Parts -Douglas Crockford</item>
        <br />
        <item>The Book of Shaders -Patricio Gonzalez Vivo, Jen Lowe</item>
        <br />
        <item>
          Advanced Binary for Programming & Computer Science -Sunil Tanna
        </item>
      </ul>
      <h5>Websites</h5>
      <item>
        <a target="_blank" href="https://distrowatch.com/">
          DistroWatch
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://slashdot.org/">
          SlashDot
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://handmade-seattle.com/">
          Handmade Seattle
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://www.corybernardi.com.au/">
          Cory Bernardi
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://lukesmith.xyz/blog">
          Luke Smith
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://sci-hub.se/">
          Sci-Hub
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://ambientcg.com/">
          Ambient CG
        </a>
      </item>

      <h5>Audio & Video</h5>
      <item>
        <a target="_blank" href="https://notrelated.xyz/">
          Not Related Podcast
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://schiffradio.com/">
          Peter Schiff Radio
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://odysee.com/">
          Odysee
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://banned.video/">
          Banned dot Video
        </a>
      </item>
      <br />
      <item>
        <a target="_blank" href="https://cozy.tv/">
          Cozy TV
        </a>
      </item>
    </div>
  );
}
